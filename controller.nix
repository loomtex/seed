      ...
      template = {
        ...
        spec = {
          ...
          initContainers = [{
            ...
            command = [ "${pkgs.openssh}/bin/ssh-keygen" "-t" "ed25519" "-f" "/tmp/ssh/id_ed25519" "-N" "" "-q" ];
            volumeMounts = [{ name = "ssh-key"; mountPath = "/tmp/ssh"; }];
          }];
          containers = [{
            name = "controller";
            image = "nix:0${cfg.controllerImage}";
            ...
            env = [
              ...
              # PATH: nix + git + coreutils + ssh (for nix eval/build and git+ssh fetching)
              { name = "PATH"; value = lib.makeBinPath [ pkgs.nix pkgs.git pkgs.coreutils pkgs.gnutar pkgs.gzip pkgs.xz pkgs.openssh ]; }
              # SSH for git+ssh:// flake fetching (ephemeral key generated by init container)
              { name = "GIT_SSH_COMMAND"; value = "ssh -i /tmp/ssh/id_ed25519 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"; }
            ...
            volumeMounts = [
              ...
              { name = "ssh-key"; mountPath = "/tmp/ssh"; readOnly = true; }
            ] ++ lib.optional (cfg.webhook.secretFile != "") {
              name = "webhook-secret";
