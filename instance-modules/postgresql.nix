...
# their own SPIFFE certs; pg_ident.conf maps certificate CNs to DB roles.
#
# Usage:
...
#       clients.worker = { role = "myapp_ro"; };
#     };
#   };
#
# The "api" instance connects with its SPIFFE cert (CN=api, OU=<namespace>).
# pg_ident maps CN "api" → role "myapp_rw" for database "myapp".
{ config, lib, pkgs, ... }:
...
let
  ...
  # Format: map_name  system_username  pg_username
  identLines = lib.concatLists (lib.mapAttrsToList (dbName: dbCfg:
    lib.mapAttrsToList (clientName: clientCfg:
      "seed  ${clientName}  ${clientCfg.role}"
    ) dbCfg.clients
  ) cfg.databases);
  ...
  # hostssl <db> all ::/0 cert map=seed
  # hostssl <db> all 0.0.0.0/0 cert map=seed
  hbaLines = lib.concatLists (lib.mapAttrsToList (dbName: dbCfg:
    lib.optionals (dbCfg.clients != {}) [
      "hostssl ${dbName} all ::/0 cert map=seed"
      "hostssl ${dbName} all 0.0.0.0/0 cert map=seed"
    ]
  ) cfg.databases);
  ...
  clientSubmodule = lib.types.submodule {
    options.role = lib.mkOption {
      type = lib.types.str;
      description = "PostgreSQL role to map this client's certificate CN to.";
    };
  };
  ...
  databaseSubmodule = lib.types.submodule {
    options.clients = lib.mkOption {
      ...
      description = ''
        ...
        specify the PostgreSQL role to map to.
      '';
    };
  };
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.postgresql = {
      ...
      # pg_ident.conf — map client cert CN → database role
      identMap = identConf;
